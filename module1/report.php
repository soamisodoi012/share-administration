...
if (isset($_GET['actionelcsh'])  == "ElectionResult")
  showElection_result_sh();
  ...
  <div class="box-body">
    ...
    <tbody>
      ...
      if ($con) {
        ...
        $query = "(SELECT res.candidate_id as id, sh.name_e as name_e, res.candidate_id as candidate_id,  SUM(res.v_value) as v_value, res.status as status, 'ALL-SHAREHOLDERS' as vote_stat
                                FROM shareholder as sh 
                                INNER JOIN elc_result as res ON sh.id=res.candidate_id
                                WHERE sh.id != '99999999'
                                AND res.status = 'ORD'
  ...
  <div class="box-body">
    ...
    <tbody>
      ...
      if ($con) {
        ...
        $query = "(SELECT res.candidate_id as id, sh.name_e as name_e, sh.name_a as name_a, res.candidate_id as candidate_id,  SUM(res.v_value) as v_value, res.status as status, 'ALL-SHAREHOLDERS' as vote_stat
                                FROM shareholder as sh 
                                INNER JOIN elc_result as res ON sh.id=res.candidate_id
                                WHERE sh.id != '99999999'
                                AND res.status = 'ORD'
                                ...
                                LIMIT 6)
          UNION ALL
                  (SELECT res.candidate_id as id, sh.name_e as name_e , sh.name_a as name_a, res.candidate_id as candidate_id,  SUM(res.v_value) as v_value, res.status as status, 'NON-INFULENTIAL' as vote_stat
                                FROM shareholder as sh 
                                INNER JOIN elc_result as res ON sh.id=res.candidate_id
                                WHERE sh.id != '99999999'
                                AND res.status = 'INF'
...
<?php
    ...
    ////////////////////////////////////////// Election result report 1 //////////////////////////////////
    function showElection_result_sh()
    {
  ...
  <tbody>
    <?php
      ...
      if ($con) {
        ...
        $query = "SELECT DISTINCT(res.sh_id) as shareholderID,sh.name_e as shName, res.v_value as shareValue, res.elc_date as elcDate 
				FROM elc_result as res
				INNER JOIN shareholder as sh ON res.sh_id=sh.id";
        $result = mysqli_query($con, $query);
        while ($row = mysqli_fetch_array($result)) {
          $seq_no++;
          showElection_resultRow_sh($row, $seq_no); //$seq_no, 
          //$total = $total + $row['subscribed_share'];
          //$totalp = $totalp + $row['paid_share']; 
          //$per = '%'; 
        }
...
<?php //colspan="5" style="color: green;"
    ...
    function showElection_resultRow_sh($row, $seq_no)
    {
?>
<tr>
  <td style='font-family:courier'><b><?php echo $seq_no; ?></b></td>
  <td style='font-family:courier'><?php echo $row['shareholderID']; ?></td>
  <td style='font-family:courier'><?php echo $row['shName']; ?></td>
  <td style='font-family:courier' class="text-right"><?php echo number_format($row['shareValue']); ?></td>
  <td style='font-family:courier'><?php echo $row['elcDate']; ?></td>
</tr>
<?php
